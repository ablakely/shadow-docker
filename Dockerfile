...
RUN apt update && apt install -y wget neofetch build-essential git libcpan-distnameinfo-perl shared-mime-info libjson-perl libxml-libxml-perl libxml-feed-perl libmojolicious-perl && git clone https://github.com/ablakely/shadow /opt/shadow && cd /opt/shadow/ && \
